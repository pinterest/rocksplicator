...
#
# 1. The default run.sh and demo_server.yaml are provided mainly for development and demo purpose.
...
# Usage example:
#
# 1. run controller in foreground
# $ sh run.sh run
#
# 2. run controller in background
# $ sh run.sh start
#
# 3. run controller in debug mode
# $ sh run.sh run --debug-app
#
# 4. stop controller running in background
# $ sh run.sh stop
#
# 5. restart controller and run in background
# $ sh run.sh restart
#
##########################################################################################
...
PID_FILE=${HOME}/rocksplicator-controller.pid
ACTION="run"
...
DEBUG_PORT=10086
JAVA_DEBUG_FLAGS="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=$DEBUG_PORT"
JAVA_CMD="java"
...
function server_start {
    ...
    OPTS="${JAVA_OPTS} \
    -cp ${CP} \
    com.pinterest.rocksplicator.controller.ControllerService \
    server ${CONFIG_FILE}"
    ...
    then
        ...
        ${JAVA_CMD} ${OPTS} &
        echo $! > ${PID_FILE}
        echo "Rocksplicator controller started."
    fi
}
...
function server_stop {
    ...
    rm -fr ${PID_FILE}
    echo "Rocksplicator controller stopped."
}
...
case $key in
    ...
    ;;
    --debug-app)
    JAVA_OPTS="$JAVA_OPTS $JAVA_DEBUG_FLAGS"
    ;;
    -h|--help)
    display_usage
	exit 0
