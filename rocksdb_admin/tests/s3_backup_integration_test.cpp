...
DEFINE_string(s3_backup_prefix, "tmp/backup_test/s3_backup_integration_test",
...
DEFINE_string(s3_backup_prefix_checkpoint,
              "tmp/backup_test/s3_backup_integration_test/checkpoint",
              "The s3 key prefix for backup checkpoint");
DEFINE_string(local_backup_dir_checkpoint,
              "/tmp/backup_test/backup/checkpoint/", "");
DEFINE_string(local_restore_dir_checkpoint,
              "/tmp/backup_test/restore/checkpoint/", "");
...
TEST(S3BackupRestoreTest, Basics) {
  ...
  for (int i = 0; i < expected_seq_no; ++i) {
    ...
    auto status = original_db->Put(
        rocksdb::WriteOptions(), "key" + to_string(i), "value" + to_string(i));
    EXPECT_TRUE(status.ok());
  }
  ...
  for (const auto& backup_with_meta : backup_with_metas) {
    for (const auto& restore_by_id : restore_by_ids) {
      LOG(INFO) << "Backup/Restore Test with backup_with_meta=" +
                       to_string(backup_with_meta) +
                       " restore_by_id=" + to_string(restore_by_id);

      // back up db. backup can be found in the s3 by using the following
      // command aws s3 ls
      // s3://FLAGS_s3_bucket/FLAGS_s3_backup_prefix/test_db_name
      auto local_s3_util = common::S3Util::BuildS3Util(50, FLAGS_s3_bucket);
      Env* backup_s3_env =
          new rocksdb::S3Env(s3_db_path, local_db_path, local_s3_util);
      ...
      if (backup_with_meta) {
        ...
        status = backup_engine->CreateNewBackupWithMetadata(original_db.get(),
                                                            db_meta);
      } else {
      ...
      // restore db
      std::string local_restored_db_path =
          FLAGS_local_restore_dir + test_db_name;
      Env* restore_s3_env =
          new rocksdb::S3Env(s3_db_path, local_restored_db_path, local_s3_util);
      unique_ptr<Env> restore_s3_env_holder(restore_s3_env);
      backup_options.backup_env = restore_s3_env;

      BackupEngine* restore_engine;
      status =
          BackupEngine::Open(Env::Default(), backup_options, &restore_engine);
      EXPECT_TRUE(status.ok());
      ...
      if (restore_by_id) {
        ...
        std::sort(backup_infos.begin(), backup_infos.end(),
                  [](BackupInfo& a, BackupInfo& b) {
                    return a.backup_id < b.backup_id;
                  });
        std::string db_meta = backup_infos.back().app_metadata;
        DBMetaData meta;

        // DBMetaData has required field db_name. Thus, it can't decode from
        // empty meta data.
        ...
        if (backup_with_meta) {
          ...
          LOG(ERROR)
              << "Failed to decode DBMetaData from backup_info.app_metadata: "
              << db_meta;
        ...
        LOG(INFO) << "Ready to restore from backupId "
                  << to_string(latest_backup_id);
        status = backup_engine->RestoreDBFromBackup(
            latest_backup_id, local_restored_db_path, local_restored_db_path);
      } else {
        status = restore_engine->RestoreDBFromLatestBackup(
            local_restored_db_path, local_restored_db_path);
      }
      EXPECT_TRUE(status.ok());

      {
        ...
        for (int i = 0; i < expected_seq_no; ++i) {
          string value;
          status =
              restored_db->Get(ReadOptions(), "key" + to_string(i), &value);
          EXPECT_TRUE(status.ok());
...
TEST(S3BackupRestoreCheckpointTest, Basics) {
  ...
  boost::filesystem::create_directories(FLAGS_local_backup_dir_checkpoint,
                                        create_err);
  boost::filesystem::remove_all(FLAGS_local_restore_dir_checkpoint, remove_err);
  boost::filesystem::create_directories(FLAGS_local_restore_dir_checkpoint,
                                        create_err);
  EXPECT_FALSE(remove_err || create_err);
  SCOPE_EXIT {
    boost::filesystem::remove_all(FLAGS_local_backup_dir, remove_err);
    boost::filesystem::remove_all(FLAGS_local_backup_dir_checkpoint,
                                  remove_err);
    boost::filesystem::remove_all(FLAGS_local_restore_dir_checkpoint,
                                  remove_err);
  };
  EXPECT_FALSE(remove_err || create_err);
  ...
  // create a new db
  std::string local_db_path = FLAGS_local_backup_dir + test_db_name;
  std::string s3_db_path =
      FLAGS_s3_backup_prefix_checkpoint + "/" + test_db_name;
  ...
  status = rocksdb::Env::Default()->GetChildren(local_db_checkpoint_path,
                                                &checkpoint_files);
  EXPECT_TRUE(status.ok());
  ...
  for (const auto& file : checkpoint_files) {
    ...
    LOG(INFO) << "PutObject from localPath: " << local_file_path
              << " to path : " << s3_file_path;
  ...
  for (auto& v : resp.Body().objects) {
    // If there is error in one file downloading, then we fail the whole restore
    // process
    auto get_resp = local_s3_util->getObject(
        v, formatted_local_path + v.substr(formatted_s3_dir_path.size()),
        false);
  ...
  status =
      rocksdb::DB::Open(rocksdb::Options(), formatted_local_path, &restored_db);
  EXPECT_TRUE(status.ok());
