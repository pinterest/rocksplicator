...
DECLARE_bool(report_gcpu_pool_stats);
...
class SafeFutureClient {
 public:
  ...
  folly::Future<typename RequestTraits<T>::response_type> future_call(
    ...
    auto evb = dynamic_cast<apache::thrift::HeaderClientChannel*>(
                   client_->getChannel())
                   ->getEventBase();
    common::Stats::get()->AddMetric(evb_queue_size_metrics_name,
 ...
 private:
  ...
  struct Callback : public apache::thrift::RequestCallback {
    explicit Callback(F&& callback) : callback_(std::forward<F>(callback)) {}
  ...
  static void process_response(
    ...
    auto ex = folly::try_and_catch<std::exception,
                                   ...
                                   typename RequestTraits<T>::exception_type>(
        [p, recvState]() {
...
template <typename ClientType>
const std::string SafeFutureClient<ClientType>::executor_enqueue_fail_times =
